...
NAME ?= ext-proc-service-aggregation
VERSION ?= $(shell git describe --tags --always --dirty 2> /dev/null || echo v0)
LD_FLAGS ?= -s -w \
	-X 'github.com/day0ops/ext-proc-service-aggregation/pkg/version.Name=${NAME}' \
	-X 'github.com/day0ops/ext-proc-service-aggregation/pkg/version.Version=${VERSION}' \
	-X 'github.com/day0ops/ext-proc-service-aggregation/pkg/version.GitCommit=${GIT_COMMIT}'
